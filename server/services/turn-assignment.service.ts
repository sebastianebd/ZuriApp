...
async function createAssignment(payload: any, currentUser: any) {
...
async function getAssignments(filters: any) {
  return await TurnAssignmentModel.find(filters)
    .populate("staffId", "firstName lastName rut positionId")
    .sort({ start_date: 1 });
}
...
async function updateAssignment(id: string, payload: any) {
