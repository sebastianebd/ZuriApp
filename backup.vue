<template>
  <main>
    <div class="row">
      ...
      <div class="col-sm-1 ms-0">
        <button @click="limpiarFechasYFiltros" class="btn btn-secondary btn-sm">
          Limpiar Filtros
        </button>
      </div>
    </div>
      ...
      <div class="table-responsive">
        <div class="row">
          <div class="col-sm-2 ms-2">
            <label for="filtroCargo" class="form-label col-form-label-sm"
              >Buscar por Rut (saliente):</label
            >

            <input
              type="text"
              v-model="filtroRutSaliente"
        ...
        <table class="table table-bordered table-sm">
          ...
          <tbody class="customtable">
            <tr v-for="(reemplazo, index) in reemplazosFiltrados" :key="index">
              ...
              <td class="small bg-success">
                {{ reemplazo.apellido_entrante }}
              </td>
          ...
          <div class="modal-body">
              ...
              <div class="col-md-6">
                <div class="border p-2 mb-2">
                  <div class="d-flex align-items-center mb-3">
                    <label
                      for="rutSaliente"
                      class="form-label form-label-sm flex-grow-1"
                      >Rut Saliente</label
                    >

                    <button
                      @click.prevent="seleccionarGrupo(1)"
                  ...
                  <div class="mb-3">
                    <input
                      type="text"
                      id="rutSaliente"
                      v-model="registroActual.rut_saliente"
                  ...
                  <div class="mb-3">
                    <label for="nombreSaliente" class="form-label form-label-sm"
                      >Nombre Saliente</label
                    >

                    <input
                      type="text"
                      id="nombreSaliente"
                      v-model="registroActual.nombre_saliente"
                      class="form-control form-control-sm"
                      disabled
                    />
                  </div>
                  ...
                  <div class="mb-3">
                    <label
                      for="apellidoSaliente"
                      class="form-label form-label-sm"
                      >Apellido Saliente</label
                    >

                    <input
                      type="text"
                      id="apellidoSaliente"
                      v-model="registroActual.apellido_saliente"
                      class="form-control form-control-sm"
                      disabled
                    />
                  </div>
              ...
              <div class="col-md-6">
                <div class="border p-2 mb-2">
                  <div class="d-flex align-items-center mb-3">
                    ...
                    <button
                      @click.prevent="seleccionarGrupo(2)"
                      class="btn btn-warning btn-sm ms-2 custom-small-button"
                    >
                      <i class="material-icons">search</i>
                    </button>
                  ...
                  <div class="mb-3">
                    <input
                      type="text"
                      id="rutSaliente"
                      v-model="registroActual.rut_entrante"
                      class="form-control form-control-sm"
                      disabled
                    />
                  </div>
                  ...
                  <div class="mb-3">
                    <label for="nombreEntrante" class="form-label form-label-sm"
                      >Nombre Entrante</label
                    >

                    <input
                      type="text"
                      id="nombreEntrante"
                      v-model="registroActual.nombre_entrante"
                      class="form-control form-control-sm"
                      disabled
                    />
                  </div>
                  ...
                  <div class="mb-3">
                    <label
                      for="apellidoEntrante"
                      class="form-label form-label-sm"
                      >Apellido Entrante</label
                    >

                    <input
                      type="text"
                      id="apellidoEntrante"
                      v-model="registroActual.apellido_entrante"
                      class="form-control form-control-sm"
                      disabled
                    />
                  </div>
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="border p-2 mb-2">
                  <div class="mb-2">
                    <label for="tipoTurno" class="form-label form-label-sm"
                      >Tipo Turno</label
                    >

                    <select
                      v-model="registroActual.tipo_turno"
                      class="form-control form-control-sm"
                    >
                      <option
                        v-for="turno in listaDeTurnos"
                        :key="turno"
                        :value="turno"
                      >
                        {{ turno }}
                  ...
                  <div class="mb-2">
                    <label for="fechaInicio" class="form-label form-label-sm"
                      >Fecha Inicio</label
                    >

                    <input
                      type="date"
                      v-model="registroActual.fecha_inicio"
                      class="form-control form-control-sm"
                    />
                  </div>

                  <div class="mb-2">
                    <label for="fechaTermino" class="form-label form-label-sm"
                      >Fecha Termino</label
                    >

                    <input
                      type="date"
                      v-model="registroActual.fecha_termino"
                      class="form-control form-control-sm"
                    />
                  </div>

                  <div class="mb-2">
                    <label for="servicio" class="form-label form-label-sm"
                      >Servicio</label
                    >

                    <select
                      v-model="registroActual.servicio"
                      class="form-control form-control-sm"
                    >
                      <option
                        v-for="servicio in listaDeServicios"
...
onMounted(async () => {
  if (!replacementStore.hayReemplazos) {
    ...
    replacementStore.mostrarReemplazos().catch(() => {});
  }

  user.value = [...replacementStore.reemplazosActivos];
  ...
  usuarios.value = await mostrarUsersCargoTens(apiPrivate);
...
const reemplazosFiltrados = computed(() => {
  ...
  if (fechaInicio.value) {
    filtradosPorFecha = filtradosPorFecha.filter(
      (reemplazo) => reemplazo.fecha_inicio >= fechaInicio.value
    );
  }

  if (fechaFin.value) {
    filtradosPorFecha = filtradosPorFecha.filter(
      (reemplazo) => reemplazo.fecha_termino <= fechaFin.value
    );
  }

  if (filtroRutSaliente.value !== "") {
    filtradosPorFecha = filtradosPorFecha.filter((reemplazo) =>
      reemplazo.rut_saliente.startsWith(filtroRutSaliente.value)
    );
  } else if (filtroRutEntrante.value !== "") {
    filtradosPorFecha = filtradosPorFecha.filter((reemplazo) =>
      reemplazo.rut_entrante.startsWith(filtroRutEntrante.value)
    );
  }

  if (filtroServicio.value !== "") {
    filtradosPorFecha = filtradosPorFecha.filter(
      (reemplazo) => reemplazo.servicio === filtroServicio.value
    );
  }
...
const usuariosFiltrados = computed(() => {
  if (filtroRut.value !== "") {
    return usuarios.value.filter((usuario) =>
      usuario.rut.startsWith(filtroRut.value)
    );
  } else {
...
const eliminar = async (index: number) => {
  await eliminarReemplazo(apiPrivate, user.value[index]._id);
...
const guardarCambios = async () => {
  ...
  user.value = await replacementStore.mostrarReemplazos();
};
